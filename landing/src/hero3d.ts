...
export function initHero3D(): void {
  ...
  const openBubble = (index: number): void => {
    ...
    const statusZh = d.status === 'resolved' ? '已解决' : '待处理'
    bubble.innerHTML =
      `<div class="h3d-bubble__head"><i>#${index + 1}</i>` +
      '<b></b>' +
      `<em class="${d.status}"></em></div>` +
      '<p></p>' +
      '<small></small>'
    const head = bubble.querySelector('b') as HTMLElement
    const em = bubble.querySelector('em') as HTMLElement
    const body = bubble.querySelector('p') as HTMLElement
    const small = bubble.querySelector('small') as HTMLElement
    setNodeText(head, d.title)
    setNodeText(em, statusZh)
    setNodeText(body, d.body)
    setNodeText(small, `Monkey_Head · ${d.author}`)
    bubble.hidden = false
    activePin = index
  }
