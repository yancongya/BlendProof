...
| C 落地页 | 未开始 | 依赖 4.2 |
| D 客户视角分层 | 未开始 | 依赖 4.3 |
| E 文案与面板 | 未开始 | 依赖 4.5、4.6 |
| （4.1）拆 review 域 | 已完成（2026-09-17） | `features/review/**` 独立；`Vec3`/`CameraState` 提升到 `shared/types/`；tsc + build + 26 后端测试 + 40 worker 测试 + 浏览器冒烟全通过 |
| （4.2）拆落地页 | 未开始 | |
| （4.3）拆 viewer 域 | 未开始 | 见上方解耦要求 |
| （4.4）拆 api 域 | 未开始 | |
| （4.5）拆 pages | 未开始 | |
| （4.6）拆 i18n | 未开始 | |
| （4.7）归位 overrides.css | 未开始 | |
